...
| Ecken | **Genau ein Radius** (`--radius-bild`, 12px) — so wie es genau vier Schriftgrößen gibt. Er gilt für Fotos, die **präsentiert** werden: Leitbild, Bildstrecke, Film. **Nicht** für das Kachelraster auf `/arbeiten`: Das ist ein *Index* und stößt absichtlich randlos aneinander wie ein Kontaktbogen — dichte, harte Kanten sind dort die Aussage. Die beiden Modi sind damit unterscheidbar: Index ist flächig, Detailseite ist Präsentation. ⚠️ **Neu am 2026-09-12.** Vorher war alles hart gekantet; Jan zur Detailseite: „nicht so kastenmäßig aneinander klatschen". |
| Leitbild (Detailseite) | **Ohne Zuschnitt** — das Bild bringt seine Form mit, begrenzt wird die **Höhe** (`72svh`, mobil `56svh`), **freigestellt auf der Bühne** statt in einem Kasten: `--buehne` als Grund, Luft ringsum, Ecken mit `--radius-bild`. ⚠️ **Der schwarze Kasten ist am 2026-09-12 gefallen.** Die erste Fassung ohne Zuschnitt setzte das Bild in ein randloses `#000`-Band über die volle Fensterbreite — bei einem Querformat standen links und rechts breite schwarze Balken, das sah aus wie ein Videoplayer im Briefmodus. Jan: „der schwarze Kasten um das Leitbild geht gar nicht." Die Lösung ist nicht, wieder zuzuschneiden, sondern den Kasten wegzunehmen: Ohne Band liegt das Foto auf derselben Fläche wie der Rest der dunklen Hälfte, und die Ränder sind kein Rahmen mehr, sondern Raum. Der Verlauf unter der Topbar (`medien-scrim`) entfällt hier mit — er säße auf der Bühne statt auf einem Foto und wäre als dunkler Streifen sichtbar. ⚠️ **Geändert am 2026-09-12; vorher `height: 72vh` + `object-fit: cover`.** Beim ersten echten Hochformat (2000×3000) fiel auf, was das anrichtet: Auf einem 1920×1080-Schirm ist die Box 1920×778, das Bild wird auf 1920×2880 skaliert und auf 778 px beschnitten — sichtbar sind **27 % des Fotos**, ein Querstreifen durch die Mitte. Dazu fordert der Browser 1920 CSS-px an (Retina 3840) und bekommt 1600 — 2,4-fach hochgerechnet. Ohne Zuschnitt ist dasselbe Hochformat nur noch ~519 CSS-px breit, die vorhandene 1200er-Stufe deckt das auf Retina mit Reserve ab: **ganzes Bild, scharf, und ein Bruchteil der Bytes.** Die Regel folgt damit derselben Begründung wie beim Porträt eine Zeile tiefer. |
...
| Bildstrecke | Waagerechter Filmstreifen in der **dunklen** Hälfte, direkt unter dem Hero — vor der Naht, damit es bei einem Wechsel pro Seite bleibt. `scroll-snap-type: x mandatory`, kein Karussell, keine Punkte, keine Pfeilknöpfe. Je Bild nur Uhrzeit und ISO. **Einheitliche Höhe, Breite nach Seitenverhältnis** — jedes Bild behält seine Form, ein Hochformat wird ein schmales hohes Feld, ein Querformat ein breites. ⚠️ **Geändert am 2026-09-12.** Vorher lag über allem ein starres `aspect-ratio: 3/2` mit `object-fit: cover` und `gap: 4px`: gleich große Kästen, fast ohne Fuge, jedes Motiv beschnitten. Die gemeinsame **Höhe** ist das, was einen Streifen zum Streifen macht — nicht ein erzwungenes gemeinsames Seitenverhältnis. Fugen jetzt `--space-md`, Ecken `--radius-bild`. Der Streifen bleibt **randlos** mit schmalem Gutter und läuft nach rechts aus dem Bild. ⚠️ Ein Zwischenstand richtete ihn an der 1040er-Textspalte aus (`max(--space-lg, (100vw − 1040px)/2)`) — geprüft bei 1440 und 1920, wo es trägt. Ab 1920 kippt es: Bei 2560 beginnt der Streifen bei 760 px und das Leitbild schon bei 502, bei 3440 liegen **1200 px links brach**. Ein randloser Scroller neben einer zentrierten Textspalte ist das ruhigere Paar; die gemeinsame Kante war eine Idee zu viel. **Merksatz: Eine Angabe, die mit `100vw` wächst, muss über die ganze Spanne geprüft werden, nicht an zwei bequemen Breiten.** |
| Film einer Arbeit | In der **dunklen** Hälfte zwischen Leitbild und Bildstrecke, in `16/9`, höchstens 1040px breit. **Mit** Bedienelementen, **ohne** Autoplay, `preload="metadata"`; Poster ist das Leitbild. Anders als das Hero ist er etwas, das man ansieht, nicht etwas, das läuft — ein Film mit Ton, der von selbst startet, ist auf einer Portfolioseite eine Zumutung. Steht **über** der Bildstrecke: Wo es einen Aftermovie gibt, ist er das stärkere Argument. |
