...
| `/arbeiten/[slug]` | Hero dunkel, Kontext hell | Vollbild-Hero mit Titel und Meta darauf → **Naht** → Aufnahmezeile und Text, links und rechts davon die Flanken → Film → Kontaktbogen → Rückweg |
...
| Bilder in der hellen Hälfte | **Erlaubt, seit dem 2026-09-13.** Die Regel „dunkel = sehen, hell = lesen“ hielt Fotos aus der hellen Hälfte heraus. Die Flanken stehen jetzt dort — und genau das ist der Punkt: Sie sind **Beleg zum Text**, nicht Bühne. Fotos auf Papierweiß neben einer Textspalte ist die Form einer Reportage, und die Seite behauptet nichts anderes. Die **eine Naht** bleibt unberührt: Es gibt weiterhin genau einen Wechsel dunkel → hell pro Seite. Von Jan ausdrücklich freigegeben. |
...
| Bildstrecke → **Flanken** | Die Bilder der Strecke stehen **links und rechts neben dem Text** in gerundeten Feldern (`--radius-bild`), die rechte Spalte um 96px versetzt. Beim Überfahren hebt sich das Feld um 8px und das Bild zoomt in seinem Rahmen — zwei getrennte Bewegungen, damit die Bildunterschrift scharf bleibt. Nur unter `@media (hover: hover) and (pointer: fine)`: Ein Finger kann nicht überfahren, und ein Wackeln beim Antippen wäre ein Effekt, der vorgibt, eine Funktion zu sein. **Mobil** fallen die Flanken unter den Text, alle Bilder **mittig**, Hochformate über `66svh` begrenzt — sonst füllt ein einzelnes Hochformat den Bildschirm allein. ⚠️ **Ersetzt den waagerechten Filmstreifen** (bis 2026-09-13). Der sperrte jedes Foto in 460px Höhe und war für ein Fotografen-Portfolio die falsche Reihenfolge — Jan: „der Slider ist gottlos beschissen“. ⚠️ **Grenze: Das Konzept hängt an der Textlänge.** Bei vielen Bildern und kurzem Text laufen die Flanken unter den Text hinaus und die Mitte wird zur leeren Rinne. Solange `content.ts` keine Beschreibungen hat, ist das der Normalfall. |
| Film einer Arbeit | In der **hellen** Hälfte **unter** dem Textblock, über die volle Breite, in `16/9`, höchstens 1040px breit. **Mit** Bedienelementen, **ohne** Autoplay, `preload="metadata"`; Poster ist das Leitbild. Anders als das Hero ist er etwas, das man ansieht, nicht etwas, das läuft — ein Film mit Ton, der von selbst startet, ist auf einer Portfolioseite eine Zumutung. ⚠️ **Stand bis zum 2026-09-13 zwischen Leitbild und Bildstrecke.** Seit die Strecke als Flanken neben dem Text steht, gibt es dort keinen Platz mehr: Ein Aftermovie ist ein eigener Auftritt, keine Flanke. |
