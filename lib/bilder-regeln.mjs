 ...
 * Formate, Qualitäten **oder die Form der Adressen** ändern — sonst werden
 * alte Ableitungen nicht neu berechnet.
 *
 * ⚠️ **Der Zusatz „Form der Adressen" steht hier, weil genau das beinahe
 * schiefgegangen wäre.** Am 2026-09-12 bekamen die Adressen einen
 * Fingerabdruck (`?v=…`) gegen einen Cache-Fehler — ohne Erhöhung dieser
 * Zahl wäre er wirkungslos geblieben: Der Cache-Schlüssel besteht aus ETag,
 * dieser Version und der Bucket-Adresse; keins davon hätte sich geändert.
 * Jedes Bild hätte als „unverändert" gegolten, und der Manifesteintrag wäre
 * samt **alter, fingerabdrucksloser Adresse** aus `stand.json` zurückgekommen
 * — der Fingerabdruck hätte nur Bilder erreicht, die ohnehin neu gerechnet
 * werden.
 *
 * Merksatz: Diese Zahl gehört nicht nur zu den **Bytes** der Ableitungen,
 * sondern zu allem, was im Manifest landet.
 */
...
/**
 * Ab hier ist eine Quelldatei auch für ein Archiv-Original zu groß.
 *
 * ⚠️ **Am 2026-09-12 deutlich angehoben** (vorher 4000 px / 6 MB). Die alten
 * Werte stammten aus der Zeit, als die Vorgabe „lange Kante 2400–3000 px"
 * hieß und die Originale im Repo lagen. Seitdem gilt beides nicht mehr: Die
 * Originale liegen im Bucket, und Jan hält sie dort **bewusst in voller
 * Größe** vor (`npm run verkleinern -- … --original`), damit die Quelle jede
 * Ausgabebreite hergibt und für einen späteren Zuschnitt Reserve bleibt.
 * Bei 4000 px hätte die Pipeline danach bei **jeder** Datei gewarnt — und
 * eine Warnung, die immer kommt, liest bald niemand mehr.
 *
 * Was bleibt, ist der Zweck: auffangen, dass versehentlich etwas völlig
 * Unpassendes im Bucket landet (ein Panorama, ein Scan, ein unkomprimiertes
 * TIFF). Rechenzeit ist dabei das eigentliche Argument — ein 8000 px breites
 * Original kostet in AVIF Minuten je Bild.
 */
export const QUELLE_WARNUNG = { kante: 8000, bytes: 40 * 1024 * 1024 };
 ...
 */
export const BUDGET = {
  warnungProMegapixel: 250 * 1024,
  fehlerProMegapixel: 900 * 1024,
};

/**
 * Das Budget einer Variante, abhängig von ihrer **Pixelzahl**.
 *
 * ⚠️ **Am 2026-09-12 von festen Zahlen auf „je Megapixel" umgestellt.** Die
 * alten Werte (200 kB Warnung, 1,5 MB harte Grenze) waren stillschweigend für
 * **Querformate** gerechnet: Bei gleicher *Breite* hat ein Hochformat die
 * 1,5-fache Pixelzahl. Ein 2400er-Hochformat ist 2400×3652 — mit rund 2,4 MB
 * hätte es die harte Grenze gerissen und den ganzen Lauf abgebrochen, obwohl
 * an der Datei nichts falsch ist. Aufgefallen beim Umstieg auf
 * Originalgrößen im Bucket.
 *
 * Die Zahlen stammen aus der Messung in `AVIF_MAX_BREITE`: Ein rauschreiches
 * Testbild — Worst Case — liegt bei rund 274 kB je Megapixel, echte Fotos
 * deutlich darunter. 900 kB/MP lässt also reichlich Luft und fängt trotzdem
 * ab, was der Fehler eigentlich meint: eine Datei ohne echte Kompression
 * (rohes RGB wären rund 3000 kB/MP).
 */
export function budgetFuer(breite, hoehe) {
  const megapixel = (breite * hoehe) / 1_000_000;
  return {
    warnung: BUDGET.warnungProMegapixel * megapixel,
    fehler: BUDGET.fehlerProMegapixel * megapixel,
  };
}
